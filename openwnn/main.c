...
void write_binary(int i,void* dic_data,NJ_UINT32 dic_size){
    //binary write
    char filename[256];
    sprintf(filename,"/tmp/con_%d.dat",i);
    FILE *fp = fopen(filename,"wb");
    if (fp==NULL){
        perror("error");
        return;
    }
    fwrite(dic_data,1,dic_size,fp);
    fclose(fp);
}
...
void get_moji(NJ_UINT8 *ptr,NJ_UINT8 siz,char* outbuf){
...
int main(int argc, const char * argv[]) {
    ...
    fclose(fp);
    //NJ_DIC_HANDLE handle=buf;
    //ptr = LEARN_INDEX_TOP_ADDR(handle);
    //printf("%f\n",GET_UINT16(ptr));
    
    //int i=0;
    //for(i=0;i<5;i++){
    //    write_binary(i,dic_data[i],dic_size[i]);
    //}
    //for(i=0;i<7;i++){
    //    write_binary(i,jdic_data[i],jdic_size[i]);
    //}
    //write_binary(0,con_01_data,sizeof(con_01_data));
    ...
    convertStringToNjChar(work->keyString,"きょう",10);
    ...
    result = ( int )njx_search_word( &( work->wnnClass ), &( work->cursor ) );
